...
-- AND inserts the initial configuration values
...
-- Replace the placeholder values with your actual .env values
-- ==========================================

INSERT INTO public.bot_config (key, value, description, category, is_secret)
VALUES 
    ('DISCORD_TOKEN', 'YOUR_DISCORD_TOKEN_HERE', 'Discord bot token', 'discord', true),
    ('CLIENT_ID', 'YOUR_CLIENT_ID_HERE', 'Discord bot client ID', 'discord', false),
    ('GUILD_ID', 'YOUR_GUILD_ID_HERE', 'Discord guild/server ID (optional)', 'discord', false),
    ('SUPABASE_URL', 'YOUR_SUPABASE_URL_HERE', 'Supabase project URL', 'supabase', false),
    ('SUPABASE_SERVICE_ROLE_KEY', 'YOUR_SUPABASE_SERVICE_ROLE_KEY_HERE', 'Supabase service role key', 'supabase', true),
    ('API_URL', 'YOUR_API_URL_HERE', 'API server URL', 'api', false),
    ('BACKEND_URL', 'YOUR_BACKEND_URL_HERE', 'Backend URL (alias for API_URL)', 'api', false),
    ('WEB_URL', 'YOUR_WEB_URL_HERE', 'Website URL', 'api', false)
ON CONFLICT (key) 
DO UPDATE SET
    value = EXCLUDED.value,
