-- Ensure moderation/subscription notification writers remain compatible with
-- environments where notifications.content is NOT NULL.
...
BEGIN
  ...
  DO UPDATE SET
    ...
    stripe_customer_id = CASE WHEN p_force_reset THEN NULL ELSE public.flair_subscriptions.stripe_customer_id END,
    stripe_subscription_id = CASE WHEN p_force_reset THEN NULL ELSE public.flair_subscriptions.stripe_subscription_id END,
    updated_at = now()
  ...
  v_message := COALESCE(NULLIF(trim(p_custom_message), ''), v_default_message);
...
BEGIN
  ...
  inserted AS (
    INSERT INTO public.notifications (
      user_id, title, message, content, type, read, metadata, created_at
    )
    SELECT
      du.user_id,
