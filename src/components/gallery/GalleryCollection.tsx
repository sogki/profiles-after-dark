...
/**
 * 
 *  Group collection of all site contents:
 *     Pfps, banners and combos
 * 
 */
...
export default function GalleryCollection({ searchQuery: externalSearchQuery, selectedCategory, selectedType, viewMode }: GalleryProps) {
  const { user } = useAuth()
  ...
  const filteredProfiles = useMemo(() => {
    return profiles.filter((profile) => {
      const matchesSearch =
        ...
        profile.category.toLowerCase().includes(internalSearchQuery.toLowerCase()) ||
      ...
      const matchesCategory =
        selectedCategory === "all" || profile.category.toLowerCase() === selectedCategory.toLowerCase()

      const matchesType = selectedType === "all" || profile.type === selectedType

      return matchesSearch && matchesTags && matchesCategory && matchesType
    })
  }, [profiles, internalSearchQuery, selectedTags, selectedCategory, selectedType])
  ...
  useEffect(() => {
    setPage(1)
  }, [internalSearchQuery, selectedTags, selectedCategory, selectedType])
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          <div className="bg-slate-800 rounded-lg p-8 max-w-md mx-auto">
            ...
            {(internalSearchQuery || selectedTags.size > 0) && (
              <button
                onClick={() => {
